...
const LANGUAGES = [
  'Vietnamese','Traditional Chinese', 'Simplified Chinese', 'Spanish', 'English', 'Hindi', 'Bengali', 'Portuguese', 'Russian',
  'Japanese', 'Punjabi', 'Marathi', 'Telugu', 'Wu Chinese', 'Turkish', 'Korean',
  'French', 'German',  'Tamil', 'Yue Chinese', 'Urdu', 'Javanese', 'Italian', 'Icelandic',
...
const SrtForm: React.FC<Props> = ({ onSubmit }) => {
  ...
  return (
    ...
    >
      ...
      >
        {file ? "✅" : "👉"} Bước 1: Chọn file SRT cần chuyển đổi
      </label>
      <div
        id="srt-file"
        onDragOver={(e) => {
      ...
      {file && (
        <>
          <div>
            ...
            >
              {language ? "✅" : "👉"} Bước 2 : Chọn ngôn ngữ cần chuyển đổi
            </label>
            <div className="rounded-lg bg-[#fafafa] text-[#444444] py-4 md:py-8 md:px-8 relative md:flex items-center text-center md:text-left">
              ...
              >
                <option value="">Chọn ngôn ngữ&hellip;</option>
          ...
          >
            Bắt đầu chuyển đổi {language ? `to ${language}` : `SRT`} &rarr;
          </button>
        </>
      )}
    </form>
